...
export default function ProductForm({ onAddProduct, textSizeClass }: ProductFormProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const readableUnit = durationType === 'days' ? 'Días' : durationType === 'months' ? 'Meses' : 'Años';
    const autoName = `Control de ${finalDurationValue} ${readableUnit}`;
  ...
  return (
    ...
    >
      ...
      {/* Product Name removed per user request */}
      ...
      <button
        type="submit"
        id="btn-save-product"
        className={`w-full py-4.5 md:py-5 px-6 rounded-2xl font-black text-center flex items-center justify-center gap-2.5 transition-all shadow-md active:scale-[0.98] bg-emerald-500 text-zinc-950 cursor-pointer hover:bg-emerald-400 shadow-lg shadow-emerald-500/10 ${textSizeClass(1.15)}`}
      >
